...
const { scanSetupEnvironment: scanHostEnvironment } = require("../environment-scan.cjs");
const { normalizeHostScan } = require("../addon-updater.cjs");
const { defaultOpenSoraPath } = require("../open-sora-paths.cjs");
const { isDiffusersWanEngine, normalizeLocalRenderEngine } = require("../local-render-engine.cjs");
const {
...
function queueBundleImport(filePath) {
  ...
  state.pendingBundleImportPath = filePath;
  state.bundlePathGuard.register(filePath);
  if (state.mainWindow && !state.mainWindow.isDestroyed()) {
    state.mainWindow.webContents.send("project:pending-bundle-import", { path: filePath });
  }
}
...
function createWindow() {
  const iconPath = path.join(state.appRoot, "icon.ico");
  const windowOptions = {
    title: `AI Video Creator v${state.pkg.version}`,
    width: 1400,
    height: 900,
    minWidth: 1100,
    minHeight: 700,
    ...
    webPreferences: {
      ...
      preload: path.join(state.appRoot, "preload.js"),
    },
  };
  ...
  const indexPath = path.join(state.appRoot, "out", "index.html");
  ...
  state.mainWindow.webContents.on("did-fail-load", (_event, code, description, url) => {
  ...
  state.mainWindow.webContents.on("did-finish-load", () => {
    if (state.pendingBundleImportPath) {
      state.mainWindow.webContents.send("project:pending-bundle-import", {
        path: state.pendingBundleImportPath,
      });
    }
  });
...
function registerBuildChild(logPath, meta, child) {
  let entry = state.activeBuilds.get(logPath);
  if (!entry) {
    entry = {
    ...
    state.activeBuilds.set(logPath, entry);
  }
...
function isBuildProcessAlive(logPath, pid) {
  const entry = logPath ? state.activeBuilds.get(logPath) : null;
...
function killBuildProcess(pid, logPath) {
  ...
  if (logPath) {
    ...
    state.activeBuilds.delete(logPath);
  }
...
function setupAddonUpdaterIpc() {
  ...
  ipcMain.handle("setup:update-addon", async (event, payload) => {
    const userDataPath = app.getPath("userData");
    const { createSetupInstallProgressBridge } = require("../setup-install-progress-bridge.cjs");
    const progress = createSetupInstallProgressBridge(event, userDataPath, { version: state.pkg.version });
  ...
  const { getInstallProtocol, scanMissingAddons, installTools } = require("../tool-installer.cjs");
...
function setupMusicVideoSyncIpc() {
  const { analyzeMusicVideoBeats } = require("../music-video-sync.cjs");
  const { assembleMusicVideo } = require("../music-video-assemble.cjs");
  ...
  ipcMain.handle("music-video:probe-ready", async () => {
    try {
      ...
      const { resolveFfmpegExecutable } = require("../music-video-assemble.cjs");
      const userDataPath = app.getPath("userData");
...
function setupAgentSessionIpc() {
  const { loadAgentSessionFile, saveAgentSessionFile } = require("../agent-session-storage.cjs");
...
function resolveCanvasIndexPath() {
  const candidates = [
    path.join(state.appRoot, "tools", "canvas", "build", "index.html"),
...
function createCanvasWindow(payload) {
  if (payload !== undefined) {
    state.pendingCanvasPayload = payload;
  }

  if (state.canvasWindow && !state.canvasWindow.isDestroyed()) {
    state.canvasWindow.focus();
    if (payload) {
      state.canvasWindow.webContents.send("canvas:payload", payload);
    }
    return { ok: true, reused: true };
  }

  const preloadPath = path.join(state.appRoot, "tools", "canvas", "electron", "preload.js");
  const iconPath = path.join(state.appRoot, "icon.ico");
  const windowOptions = {
  ...
  if (!app.isPackaged && devUrl) {
    ...
    if (!indexPath) {
      ...
      state.canvasWindow.destroy();
      state.canvasWindow = null;
      return { ok: false, error: "Canvas build missing" };
    }
    state.canvasWindow.loadFile(indexPath);
  }

  state.canvasWindow.on("closed", () => {
    state.canvasWindow = null;
  });
...
function setupCanvasIpc() {
  ...
  ipcMain.handle("canvas:get-payload", async () => {
    const payload = state.pendingCanvasPayload;
    state.pendingCanvasPayload = null;
    return payload;
  });
  ...
  ipcMain.handle("canvas:request-refresh", async () => {
    try {
      if (state.mainWindow && !state.mainWindow.isDestroyed()) {
        state.mainWindow.webContents.send("canvas:refresh-requested");
        return { ok: true };
      }
  ...
  ipcMain.handle("canvas:get-version", async () => {
    return { ok: true, version: state.pkg.version };
  });
}
...
function setupProjectImportIpc() {
  ipcMain.handle("project:read-bundle-file", async (_event, filePath) => {
    try {
      const pathCheck = state.bundlePathGuard.validate(filePath, {
        pendingPath: state.pendingBundleImportPath,
      });
      if (!pathCheck.ok) {
        return pathCheck;
      }
  ...
  ipcMain.handle("project:consume-pending-bundle", async () => {
    const current = state.pendingBundleImportPath;
    state.pendingBundleImportPath = null;
    if (current) {
      state.bundlePathGuard.register(current);
...
function setupAutoUpdater() {
  ...
  try {
    ...
    autoUpdater.on("update-available", () => {
      if (state.mainWindow && !state.mainWindow.isDestroyed()) {
        state.mainWindow.webContents.send("app-update-status", { status: "available" });
      }
    });

    autoUpdater.on("update-downloaded", () => {
      if (state.mainWindow && !state.mainWindow.isDestroyed()) {
        state.mainWindow.webContents.send("app-update-status", {
          status: "downloaded",
    ...
    autoUpdater.on("update-not-available", () => {
      if (state.mainWindow && !state.mainWindow.isDestroyed()) {
        state.mainWindow.webContents.send("app-update-status", { status: "not-available" });
      }
    });
...
function openReadmeOnce() {
  ...
  const possiblePaths = [
    ...
    path.join(state.appRoot, "build", "AI_Video_Creator_README.pdf"),
    path.join(state.appRoot, "AI_Video_Creator_README.pdf"),
  ];
